...
rddpron=sc.textFile("je.csv")
rddpronclient=sc.textFile("pronomsclient.txt")
...
##########################################################identite du chatbot#############################################################################
#rddpron1=rddpron.map(amine)
#rddpron2=rddpron1.lookup('chatbot')
rddpron="je"
...
for i in proc:
		...
		if (rddsentiments2!=[]):
			...
			print rddpron +" ne suis pas sentimental "  
...
#pronom(chatbot) "suis pas" $"affection" 
